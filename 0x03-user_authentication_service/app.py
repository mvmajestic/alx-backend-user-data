...
OBOBOB    """
OBOBOB    email = request.form.get("email")
    try:
OBOBOB        reset_token = AUTH.get_reset_password_token(email)
    except ValueError:
        abort(403)
OBOBOB
OBOBOB    return jsonify({"email": f"{email}", "reset_token": f"{reset_token}"})
OBOBOB
OBOBOB
...
OBOBOB    app.run(host="0.0.0.0", port="5000")
